...
int main(int argc, char* argv[])
{
	...
	while(!sorted)
    	{
		sorted = 1;
		for(odd_even = 0 ; odd_even < 2 && rank<size ; ++odd_even)
		{
			if(rank%2==odd_even)
			{
				if(rank<size-1)
				{
					MPI_Isend(data+dataSize/2, dataSize/2+dataSize%2, MPI_FLOAT, rank+1, 0, MPI_COMM_WORLD, &req1);
					MPI_Isend(data, dataSize/2, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD, &req3);

					MPI_Irecv(right, rightSize/2+rightSize%2, MPI_FLOAT, rank+1, 0, MPI_COMM_WORLD, &req2);
					MPI_Irecv(right+rightSize/2+rightSize%2, rightSize/2, MPI_FLOAT, rank+1, 1, MPI_COMM_WORLD, &req4);
			...
			else
			{
				if(rank > 0)
				{
					MPI_Isend(data, dataSize/2+dataSize%2, MPI_FLOAT, rank-1, 0, MPI_COMM_WORLD, &req1);
					MPI_Isend(data+dataSize/2+dataSize%2, dataSize/2, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD, &req3);

					MPI_Irecv(left+leftSize/2, leftSize/2+leftSize%2, MPI_FLOAT, rank-1, 0, MPI_COMM_WORLD, &req2);
					MPI_Irecv(left, leftSize/2, MPI_FLOAT, rank-1, 1, MPI_COMM_WORLD, &req4);
